from pyspark.sql import SparkSession
from pyspark.sql.functions import lit, sum, col, coalesce, round
from pyspark.sql import DataFrame


def get_spark_session():
    spark = SparkSession.builder \
        .appName("TMSTransOrderStatsETL") \
        .config("hive.metastore.uris", "thrift://cdh01:9083") \
        .config("spark.sql.hive.convertMetastoreOrc", "true") \
        .enableHiveSupport() \
        .getOrCreate()
    sc = spark.sparkContext
    sc.setLogLevel("WARN")
    spark.sql("USE tms")
    return spark


def write_to_ads_trans_order_stats(df: DataFrame):
    try:
        target_table = "tms.ads_trans_order_stats"
        spark = get_spark_session()
        target_df = spark.table(target_table)
        df = df.select([col(c).cast(target_df.schema[c].dataType) for c in target_df.columns])
        df.write \
            .mode("append") \
            .insertInto(target_table)
        print(f"[INFO] 成功写入 {df.count()} 条新数据到 {target_table}")
    except Exception as e:
        print(f"[ERROR] 写入失败：{str(e)}")
        print("[ERROR] 待写入数据样例：")
        df.show(5)
        raise e


def execute_trans_order_stats_etl(target_date: str):
    spark = get_spark_session()
    print(f"[INFO] 开始执行运输订单统计ETL，目标日期：{target_date}")

    # 处理1天数据
    receive_1d = spark.table("tms.dws_trans_org_receive_1d") \
        .filter(col("dt") == target_date) \
        .agg(
        sum("order_count").alias("receive_order_count"),
        sum("order_amount").alias("receive_order_amount")
    ) \
        .withColumn("recent_days", lit(1))

    dispatch_1d = spark.table("tms.dws_trans_dispatch_1d") \
        .filter(col("dt") == target_date) \
        .select(
        lit(1).alias("recent_days"),
        col("order_count").alias("dispatch_order_count"),
        col("order_amount").alias("dispatch_order_amount")
    )

    part1d = receive_1d \
        .join(dispatch_1d, on="recent_days", how="full_outer") \
        .select(
        lit(target_date).alias("dt"),
        coalesce(receive_1d["recent_days"], dispatch_1d["recent_days"]).alias("recent_days"),
        col("receive_order_count"),
        col("receive_order_amount"),
        col("dispatch_order_count"),
        col("dispatch_order_amount")
    )

    # 处理多天数据
    receive_nd = spark.table("tms.dws_trans_org_receive_nd") \
        .filter(col("dt") == target_date) \
        .groupBy("recent_days") \
        .agg(
        sum("order_count").alias("receive_order_count"),
        sum("order_amount").alias("receive_order_amount")
    )

    dispatch_nd = spark.table("tms.dws_trans_dispatch_nd") \
        .filter(col("dt") == target_date) \
        .select(
        col("recent_days"),
        col("order_count").alias("dispatch_order_count"),
        col("order_amount").alias("dispatch_order_amount")
    )

    partnd = receive_nd \
        .join(dispatch_nd, on="recent_days", how="full_outer") \
        .select(
        lit(target_date).alias("dt"),
        coalesce(receive_nd["recent_days"], dispatch_nd["recent_days"]).alias("recent_days"),
        col("receive_order_count"),
        col("receive_order_amount"),
        col("dispatch_order_count"),
        col("dispatch_order_amount")
    )

    # 合并数据
    current_data = spark.table("tms.ads_trans_order_stats") \
        .select(
        col("dt"),
        col("recent_days"),
        col("receive_order_count"),
        col("receive_order_amount"),
        col("dispatch_order_count"),
        col("dispatch_order_amount")
    )

    final_df = current_data.unionByName(part1d).unionByName(partnd)

    # 按目标表列顺序整理
    target_columns = [
        "dt", "recent_days",
        "receive_order_count", "receive_order_amount",
        "dispatch_order_count", "dispatch_order_amount"
    ]
    final_df = final_df.select(target_columns)

    final_df.show(5)
    # 写入目标表
    write_to_ads_trans_order_stats(final_df)
    print(f"[INFO] ETL执行完成，目标日期：{target_date}")


if __name__ == "__main__":
    target_date = '2025-07-11'
    execute_trans_order_stats_etl(target_date)