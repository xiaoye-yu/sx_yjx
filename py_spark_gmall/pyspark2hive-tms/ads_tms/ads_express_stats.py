...
def write_to_ads_express_stats(df: DataFrame):
    """写入目标表ads_express_stats，增加类型校验和错误处理"""
    try:
        # 1. 校验目标表结构
        target_table = "tms.ads_express_stats"
...
def execute_express_stats_etl(target_date: str):
    spark = get_spark_session()
    print(f"[INFO] 开始执行快递统计ETL，目标日期：{target_date}")

    # 1. 处理1天数据（1d表）
    # 1.1 派送成功数据
    deliver_1d = spark.table("tms.dws_trans_org_deliver_suc_1d") \
    ...
    # 1.2 分拣数据
    sort_1d = spark.table("tms.dws_trans_org_sort_1d") \
        .filter(col("dt") == target_date) \
    ...
    # 2. 处理多天数据（nd表）
    # 2.1 派送成功数据
    deliver_nd = spark.table("tms.dws_trans_org_deliver_suc_nd") \
        .filter(col("dt") == target_date) \
        .groupBy("recent_days") \
        .agg(sum("order_count").alias("deliver_suc_count")) \
        .withColumn("dt", lit(target_date))
    ...
    # 2.2 分拣数据
    sort_nd = spark.table("tms.dws_trans_org_sort_nd") \
        .filter(col("dt") == target_date) \
        .groupBy("recent_days") \
        .agg(sum("sort_count").alias("sort_count")) \
        .withColumn("dt", lit(target_date))
